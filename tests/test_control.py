...
@pytest.mark.usefixtures("available_envs")
def test_sarsa_lambda_linear(available_envs: list) -> None:
    """Test SARSA(lambda) control with linear VFA."""
    for env in available_envs:
        env = EnvironmentNormalizer(env)
        n_states = env.observation_space.shape[0]
        q_function = QLinear(
...
@pytest.mark.usefixtures("available_envs")
def test_sarsa_lambda_tabular(available_envs: list) -> None:
    """Test SARSA(lambda) control with tabular Q."""
    for env in available_envs:
        env = EnvironmentNormalizer(env)
        n_states = env.observation_space.shape[0]
        q_function = QTabular(
